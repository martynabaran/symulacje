...
class Fire:
    ...
    def spread(self, walls, grid_width, grid_height):
        """
        Spreads the fire in random directions every 'spread_rate' steps.
        Fire does not spread through walls.
        """
        if self.current_step % self.spread_rate == 0:
            new_positions = set()
            ...
            for pos in self.positions:
                # Losowo wybieramy liczbę kierunków (od 1 do 4), w które ogień się rozprzestrzeni
                directions = random.sample([(1, 0), (-1, 0), (0, 1), (0, -1)], random.randint(1, 4))

                # Dodatkowa losowa szansa na rozprzestrzenienie w tym kroku (np. 50%)
                if random.random() < 0.5:  
                    continue  # W tym kroku ogień się nie rozprzestrzenia z tego punktu
                
                for dx, dy in directions:
                    new_pos = Point(pos.x + dx, pos.y + dy)
                    
                    # Sprawdzamy, czy nowa pozycja jest w granicach siatki i nie ma tam ściany
                    if 0 <= new_pos.x < grid_width and 0 <= new_pos.y < grid_height:
                        if not self.is_blocked_by_wall(new_pos, walls):
                            new_positions.add(new_pos)

            # Aktualizacja pozycji ognia
            self.positions.update(new_positions)

        # Zwiększamy licznik kroków
        self.current_step += 1
...
class Environment():
    ...
    def __init__(self, N, walls, goals, agents, conditions, instruments, fire=None):
        self.N = N
        self.walls = walls
        self.goals = goals
        self.agents = agents
    ...
    def step(self):
        ...
        if self.fire:
            self.fire.spread(self.walls, self.N, self.N)
...
class EnvironmentViewer():
    ...
    def drawFire(self, fire):
        """ Rysowanie ognia na ekranie """
        for pos in fire.positions:
            pygame.draw.rect(self.screen, self.FIRE_COLOR, pygame.Rect(pos.x * self.pygameScale, pos.y * self.pygameScale, self.pygameScale, self.pygameScale))
...
# TODO
# add option to initilize starting fire position
def runSimulation(roomHeight=10,
    ...
    fire = Fire(initial_positions=[Point(2,2)], spread_rate=200)
    ...
    for _ in range(numAgents):
        ...
        agents.append(Agent(size, mass, pos, goals, desiredSpeed=desiredSpeed, fires=fire))
    
    env = Environment(100, walls, goals, agents, {}, instruments, fire=fire)
